...
pub fn build_tile_with_modifier(random: bool, is_or: bool, negated: bool, direction: Option<WantsToMove>, sprites: &Vec<SpriteState>) -> TileWithModifier {
    ...
    let tile = Tile::new(tile_kind, &String::from("autogenerated_name"), sprites.clone());
...
pub fn build_t(random: bool, sprite: &SpriteState, negated: bool, dir: Option<WantsToMove>) -> TileWithModifier {
...
impl AddTile {
    fn evaluate<R: Rng + ?Sized>(&self, rng: &mut R, board: &mut Board, pos: &Position, magic_or_tiles: &FnvHashMap<Tile, Vec<SpriteState>>) -> bool {
        ...
        if t.random {
            ...
            trace!("Adding random sprite={} dir={} to {:?}. Did it? {}", sprites[rnd], dir, pos, ret);
            return ret
        }
        ...
        match t.tile.kind {
            TileKind::Or => {
                ...
                match magic_or_tiles.get(&t.tile) {
                    None => panic!("Magic OR tile '{}' not found. Here are all the tiles: {:?}", t.tile.name, magic_or_tiles),
                    Some(sprites) => {
                        for sprite in sprites {
                            trace!("Adding Magic sprite={} to {:?}", sprite, pos);
                            something_evaluated |= board.add_sprite(pos, sprite, get_dir(rng, self.new_direction));
                        }
                    }
                }
            },
            TileKind::And => {
                for sprite in t.tile.get_sprites() {
                    ...
                    something_evaluated |= board.add_sprite(pos, sprite, self.new_direction.unwrap_or(current_direction.unwrap_or(WantsToMove::Stationary)));
                }
            }
        }
        something_evaluated
...
impl RemoveTile {
    fn evaluate(&self, board: &mut Board, pos: &Position, or_tiles_removed: &mut FnvHashSet<Tile>) -> bool {
        ...
        if self.condition_sprites_to_remove.tile.is_or() {
            if !or_tiles_removed.contains(&self.condition_sprites_to_remove.tile) {
                ...
                let tile = SpriteState::new(&String::from("temp-removetile"), tile.unwrap().sprite_index, self.collision_layer); // just build a sprite

                // only remove the sprites in the cell that match the condition... not all the sprites in a collisionLayer
                let cell_sprite = tile;
                
                for condition_sprite_to_remove in self.condition_sprites_to_remove.tile.get_sprites() {
                    if cell_sprite == *condition_sprite_to_remove {
                        let removed = board.remove_collision_layer(pos, cell_sprite.collision_layer);
...
impl ReplaceDirection {
    ...
    fn evaluate<R: Rng + ?Sized>(&self, rng: &mut R, board: &mut Board, pos: &Position) -> bool {
        ...
        trace!("Replacing direction on {} to be {:?}", self.collision_layer, self.new_direction);
        board.set_wants_to_move(pos, self.collision_layer, get_dir(rng, Some(self.new_direction)))
    }
...
impl Neighbor {
    pub fn new(tiles_with_modifier: Vec<TileWithModifier>) -> Self {
        ...
        for t in &tiles_with_modifier {
            if t.negated {
                match t.tile.kind {
                    TileKind::And => if t.tile.sprites.len() == 1 { no_sprites.insert_all(&t.tile.bits) },
    ...
    fn check4(&self, cell_map: &FnvHashMap<u16, SpriteAndWantsToMove>) -> bool {
        for (c, dir) in &self.sprite_movements_present {
            match cell_map.get(&c) {
                None => return false,
                Some(sw) => if &sw.wants_to_move != dir { return false },
            }
        }
        true
    }
        ...
        for (tile, dir) in &self.or_tile_movements_present {
            if tile.has_single_collision_layer() {
                ...
                match cell_map.get(set.iter().next().unwrap()) {
                    None => return false,
                    Some(sw) => if dir != &sw.wants_to_move { return false },
                }
            } else {
                ...
                for sprite in &tile.sprites {
                    if cell_bits.contains(sprite.index) {
                        ...
                        match cell_map.get(&sprite.collision_layer) {
                            None => {},
                            Some(sw) => { if dir == &sw.wants_to_move { did_match = true; } },
                        }
                    }
                }
                if !did_match {
                ...
                assert!(found_sprite, "BUG: Could not find sprite. One should have already been matched before");
            }
        }
        true
    }
    ...
    pub fn matches(&self, board: &Board, pos: &Position) -> bool {
        ...
        let debug_check = || {
            ...
            if ret != naive {
                eprintln!("neighbor... {}, pos: {} cell:{:?}", self, pos, board.as_map(&pos));
                eprintln!("Expected... {} but found {} {} {} {} {} {}", naive, self.check1(cell_sprites), self.check2(cell_sprites), self.check3(cell_sprites), self.check4(board.as_map(pos)), self.check5(cell_sprites, board.as_map(pos)), self.check6(board, pos));
            }
            ret == naive
        };
            ...
            if condition.is_some() && action.is_some() {
                ...
                if condition != action { // Could be `[ TrolleyFull no CleanDishes] -> [TrolleyEmpty no CleanDishes ]`
                    ...
                    if condition.direction != action.direction || condition.negated {
                        new_direction = Some(action.direction.clone().unwrap_or(WantsToMove::Stationary));
                    } else {
                        new_direction = None;
                    }
                ...
                let action = action.unwrap();
                if !action.negated {
                    let new_direction = Some(action.direction.clone().unwrap_or(WantsToMove::Stationary));
                    self.add_tiles.push(AddTile {
                        collision_layer: c,
                        action_tile_with_modifier: action,
                        new_direction
    ...
    pub fn evaluate<R: Rng + ?Sized>(&self, rng: &mut R, board: &mut Board, pos: &Position, magic_or_tiles: &FnvHashMap<Tile, Vec<SpriteState>>) -> bool {
        assert!(self.prepared_actions, "Forgot to call prepare_actions on the Neighbor");
        debug_assert!(self.matches(board, pos), "Should have already checked to see if this neighbor matches");
    ...
    fn pair_up_tiles(&self, action: &Neighbor) -> FnvHashMap<u16, ExtraPair<Option<TileWithModifier>, bool>> {
        ...
        for t in &self.tiles_with_modifier {
            ...
            if t.tile.is_or() && !t.tile.has_single_collision_layer() {
                ...
                for c in t.tile.get_collision_layers() {
                    pairs.entry(*c).or_insert(ExtraPair::new(Some(t.clone()), None, false));
                }
            }
        }
        ...
        for t in &action.tiles_with_modifier {
            if t.tile.is_or() && !t.tile.has_single_collision_layer() {
                ...
                match or_tiles.get(&t.tile) {
                        ...
                        if condition_tile.direction != t.direction {
                            for sprite in t.tile.get_sprites() {
                                ...
                                if !pairs.contains_key(&c) {
                                    pairs.insert(c, ExtraPair::new(
                                        Some(build_t(condition_tile.random, sprite, condition_tile.negated, condition_tile.direction)),
                    ...
                    None => {
                        let t2 = TileWithModifier { negated: false, random: t.random, direction: t.direction, tile: t.tile.clone(), };
                        if t.negated {
                            for sprite in t.tile.get_sprites() {
                                let c = sprite.collision_layer;
                                if !pairs.contains_key(&c) {
                                    pairs.insert(c, ExtraPair::new(Some(t2.clone()), None, true/*okToIgnoreNonMatches*/));
                                }
                            }
                        } else {
                            for sprite in t.tile.get_sprites() {
                                let c = sprite.collision_layer;
                                if !pairs.contains_key(&c) {
                                    pairs.insert(c, ExtraPair::new(None, Some(t2.clone()), true/*okToIgnoreNonMatches*/));
                                }
                            }
                        }
                    }
    ...
    pub fn populate_magic_or_tiles(&self, magic_or_tiles: &mut FnvHashMap<Tile, Vec<SpriteState>>, board: &Board, pos: &Position) {
        for t in &self.tiles_with_modifier {
            if !t.negated && t.tile.is_or() {
                let sprites = t.tile.get_sprites().iter().filter(|s| board.has_sprite(pos, s)).map(|s| *s).collect();
   ...
   pub fn populate_cache(&self, all: &mut BitSet, any: &mut BitSet, sprite_movements_present: &mut FnvHashSet<(u16, WantsToMove)>) {
...
pub mod tests {
    ...
    pub fn check_counts(condition: &Neighbor, direction: usize, added: usize, removed: usize) {
        assert_eq!(direction, condition.replace_directions.len(), "Direction Mismatched: (expected, actual)");
        assert_eq!(added, condition.add_tiles.len(), "AddTiles Mismatched: (expected, actual)");
        assert_eq!(removed, condition.remove_tiles.len(), "RemoveTiles Mismatched: (expected, actual)");
    }
    ...
    fn tiles_with_modifier() {
        ...
        assert!(build_tile_with_modifier(false/*random*/, false/*is_or*/, false /*negated*/, None, &vec![player_sprite]).matches(&board, &origin));
        // Check that tile negation works
        assert!(!build_tile_with_modifier(false/*random*/, false/*is_or*/, true /*negated*/, None, &vec![player_sprite]).matches(&board, &origin));

        // Check that the player is NOT stationary
        assert!(!build_tile_with_modifier(false/*random*/, false/*is_or*/, false /*negated*/, Some(WantsToMove::Stationary), &vec![player_sprite]).matches(&board, &origin));
        // Check that the player is moving Right
        assert!(build_tile_with_modifier(false/*random*/, false/*is_or*/, false /*negated*/, Some(WantsToMove::Right), &vec![player_sprite]).matches(&board, &origin));

        // Verify that crates and targets do NOT match
        assert!(!build_tile_with_modifier(false/*random*/, false/*is_or*/, false /*negated*/, None, &vec![crate_sprite]).matches(&board, &origin));
        assert!(!build_tile_with_modifier(false/*random*/, false/*is_or*/, false /*negated*/, None, &vec![target_sprite]).matches(&board, &origin));

        // Verify that "Player AND Target" does NOT match
        assert!(!build_tile_with_modifier(false/*random*/, false/*is_or*/, false /*negated*/, None, &vec![player_sprite, target_sprite]).matches(&board, &origin));

        // Verify that "Player OR Target" does match
        assert!(build_tile_with_modifier(false/*random*/, true/*is_or*/, false /*negated*/, None, &vec![player_sprite, target_sprite]).matches(&board, &origin));

        // Verify that "Player OR Crate" matches
        assert!(build_tile_with_modifier(false/*random*/, true/*is_or*/, false /*negated*/, None, &vec![player_sprite, crate_sprite]).matches(&board, &origin));

        // Ensure subsequent checks are possible
        let t = build_tile_with_modifier(false/*random*/, false/*is_or*/, false /*negated*/, None, &vec![player_sprite]);
    ...
    fn replace_direction() {
        ...
        let mut condition = Neighbor::new(vec![build_t(false/*random*/, &player, false/*negated*/, None)]);
        let action = Neighbor::new(vec![build_t(false/*random*/, &player, false/*negated*/, Some(WantsToMove::Right))]);
    ...
    fn matches_no_or() {
        ...
        // thing = player OR bat
        // [ NO thing ]
        let no_thing = build_tile_with_modifier(false/*random*/, true/*is_or*/, true /*negated*/, None, &vec![player, bat]);
    ...
    fn random_dir() {
        ...
        let player_random = build_t(false/*random*/, &player, false, Some(WantsToMove::RandomDir));
        ...
        assert_eq!(board.get_wants_to_move(&origin, player.collision_layer), Some(WantsToMove::Up)); // The direction is based on RNG
    }
    ...
    fn remove_one_or_sprite() {
        ...
        let thing_any = build_tile_with_modifier(false, true, false, None, &vec![player.clone(), hat.clone()]);
    ...
    fn matches_two_ors() {
        ...
        // playerish = player OR ...
        // batish = bat OR ...
        let playerish = build_tile_with_modifier(false/*random*/, true/*is_or*/, false/*negated*/, None, &vec![player]);
        let batish = build_tile_with_modifier(false/*random*/, true/*is_or*/, false/*negated*/, None, &vec![bat]);
