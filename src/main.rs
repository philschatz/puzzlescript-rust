...
fn play_game<B: Backend>(terminal: &mut Terminal<B>, path: &str, start_level: Option<u8>, scripted: bool, enable_sound: bool, no_flick_screen: bool, tick_speed: Option<u64>) -> Result<(), Box<Error>> {
    ...
    let mut attribution = Attribution::new(game.title.clone(), game.metadata.author.clone(), game.metadata.homepage.clone());
    ...
    let save_game = |current_level_num: u8, inputs: Vec<String>, board: Option<Board>| -> Result<(), Box<Error>> {

        let checkpoint = board.map(|board| board.positions_iter().iter()
            .map(|p| {
                board.get_sprite_states(&p).iter()
    ...
    loop {
        ...
        match key {
            Ok(key) => {
                ...
                should_tick = match key {
                    ...
                    | Key::Char('q') => break,
                    Key::Char('?') 
                    | Key::Char('h') => { help.toggle(); false }
                    Key::Up
                    | Key::Char('w')
                    | Key::Char('W') => { input = input.or(Some(EngineInput::Up)); keys += 1; true },
                    Key::Down
                    | Key::Char('s')
                    | Key::Char('S') => { input = input.or(Some(EngineInput::Down)); keys += 1; true },
                    Key::Left
                    | Key::Char('a')
                    | Key::Char('A') => { input = input.or(Some(EngineInput::Left)); keys += 1; true },
                    Key::Right
                    | Key::Char('d')
                    | Key::Char('D') => { input = input.or(Some(EngineInput::Right)); keys += 1; true },
                    Key::Char(' ')
                    | Key::Char('\n')
                    | Key::Char('x')
                    | Key::Char('X')
                    | Key::Char('!') => { input = input.or(Some(EngineInput::Action)); keys += 1; true },
                    Key::Char('z')
                    | Key::Char('Z')
                    | Key::Char('u') => { input = input.or(Some(EngineInput::Undo)); keys += 1; true },
                    Key::Char('R')
                    | Key::Char('r') => { input = input.or(Some(EngineInput::Restart)); keys += 1; true },
                    Key::Char('p') => { if !was_paused { play_pause.pause() }; false },
                    Key::Char('c') => { terminal.draw(|_|{})?/*repaint*/; false },
                    // Solution files keys. These are not pressed, they are piped in
                    Key::Char('#') => false,
                    Key::Char('.')
                    | Key::Char(',') => true,
                    Key::Null => { println!("Done reading input"); process::exit(111) },
                    // Debugging
                    Key::Char('~')
                    | Key::Char('`')
                    | Key::Char('\\') => { 
            ...
            Err(TryRecvError::Empty) => {
                if scripted {
                    if keys > 0 {
                        ...
                        panic!("BUG: Level ended but no keys were pressed. Maybe more input is needed");
                    }
                }
                should_tick = !engine.debug_rules; // Do not tick when debugger is on.

                // Pause the game when idle (only for realtime games)
                if !play_pause.paused && engine.game_data.metadata.realtime_interval.is_some() && last_input.elapsed().as_secs() > IDLE_SECS {
                    play_pause.pause();
                    should_tick = false;
                }
        ...
        if tr.changed {
            add_input(&mut inputs, engine.current_level_num, input.map(|i| i.to_key()).unwrap_or('.'));
        }
...
fn clear_screen() {
    print!("{}{}{}", termion::color::Fg(termion::color::Reset), termion::color::Bg(termion::color::Reset), termion::clear::All);
}
fn play_bell() {
    print!("\x07")
}
